...
void SensorManager::task(void *param) {
	...
	for (uint8_t g = 0; g < self->_groupCount; ++g) {
		self->_groups[g].nextDueMs = now;
		for (uint8_t s = 0; s < self->_groups[g].sensorCount; ++s) {
			self->_groups[g].sensors[s]->bind();
		}
	}
...
void SensorManager::pollDueGroups() {
	uint32_t now = HAL_GetTick();
	for (uint8_t g = 0; g < _groupCount; ++g) {
		SensorGroup &group = _groups[g];

		if (group.nextDueMs > now)
			continue;

		for (uint8_t s = 0; s < group.sensorCount; ++s) {
			ISensor *sensor = group.sensors[s];
			if (sensor == nullptr)
				continue;

			sensor->trigger();
			uint32_t flag = sensor->doneFlag();
			while (flag != 0) {
				uint32_t bits = 0;
		...
		now = HAL_GetTick();
		group.nextDueMs += group.periodMs;
		while (group.nextDueMs <= now)
			group.nextDueMs += group.periodMs;
	}
}
