...
public:
	...
	enum class QueuePolicy : uint8_t {
		OVERWRITE, DROP_SILENT
	};
	struct BusConfig {
		Topic topic;
		QueuePolicy policy;
	};
	static constexpr BusConfig BUS_CONFIG[] = { { Topic::TELEMETRY, QueuePolicy::OVERWRITE }, {
		Topic::ALERT, QueuePolicy::OVERWRITE }, { Topic::HEALTH, QueuePolicy::DROP_SILENT }, {
		Topic::LOG, QueuePolicy::DROP_SILENT }, };

	ExternalComm(ICommChannel *uart, ICommChannel *usb, ICommChannel *eth,
		IActuatorManager *actuatorMgr, QueueHandle_t motionMailbox,
		Config::ComQueuePolicy uartPolicy = Config::UART_POLICY, Config::ComQueuePolicy usbPolicy =
			Config::USB_POLICY, Config::ComQueuePolicy ethPolicy = Config::ETH_POLICY);
	...
	QueueHandle_t rxByteQueue() const {
		return _rxByteQueue;
	}
