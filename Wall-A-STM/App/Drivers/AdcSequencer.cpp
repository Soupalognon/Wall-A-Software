...
AdcSequencer::AdcSequencer(ADC_HandleTypeDef *hadc, uint32_t doneFlag,
                           const uint32_t *channels, uint8_t maxChannel,
                           uint16_t *rawValues)
    : _hadc(hadc), _doneFlag(doneFlag), _channels(channels),
      _maxChannel(maxChannel), _rawValues(rawValues) {}
