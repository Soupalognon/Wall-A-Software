...
AnalogSensor::AnalogSensor(uint8_t id, const char *name, IAnalogSource *src, uint8_t channel,
	float alarmThreshold) :
	_id(id), _name(name), _src(src), _channel(channel), _alarmThreshold(alarmThreshold) {
}
